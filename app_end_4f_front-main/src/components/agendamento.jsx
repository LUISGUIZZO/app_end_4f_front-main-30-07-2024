...
const Agendamento = () => {
    ...
    const handleServicoChange = (event) => {
        ...
        const servicoEncontrado = servicos.find(servico => servico.servicoId === parseInt(event.target.value, 10));        console.log("Servico encontrado:", servicoEncontrado); // Log do serviço encontrado
                    ...
                    <form onSubmit={handleSubmit(salvar)}>
                        ...
                        >
                            ...
                            {servicos.map(servico => (
                                <option key={servico.servicoId} value={servico.servicoId}>{servico.servicoNome}</option>
                            ))}
                        </select>
                        <br />
                        <select className="form-select" aria-label="Prestador" {...register("prestador_id")} defaultValue="" disabled={!selectedServicoNome}>
